...
public class RestAuthenticationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filter)
		...
		if (request instanceof HttpServletRequest) {
			...
			} else if (authCredentials != null && !authCredentials.isEmpty())
			// Get Version
			{
				if (path.contains("login") || path.endsWith("split") || path.endsWith("credit")
						|| path.endsWith("payment/trans") || path.contains("deviceidbg")
						|| path.contains("customer/get") || path.contains("customer/cpwd")
						|| path.contains("customer/guest")|| path.contains("journal/jauth")) {
